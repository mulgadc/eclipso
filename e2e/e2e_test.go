...
const (
	eclipsoUDP  = "127.0.0.1:5553"
	eclipsoTCP  = "127.0.0.1:5553"
	eclipsoDoT  = "127.0.0.1:8853"
...
func TestMain(m *testing.M) {
	if os.Getenv("ECLIPSO_E2E") == "" {
		fmt.Println("Skipping E2E tests (set ECLIPSO_E2E=1 to run)")
		os.Exit(0)
	}
	...
	// Wait for eclipso to sync zone files from S3
	fmt.Println("Waiting for eclipso to sync zone files...")
	if err := waitForDNS(30 * time.Second); err != nil {
		fmt.Printf("Eclipso DNS not ready: %v\n", err)
		os.Exit(1)
	}
...
func waitForDNS(timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		r, _, err := c.Exchange(&m, eclipsoUDP)
		if err == nil && r.Rcode == dns.RcodeSuccess && len(r.Answer) > 0 {
			fmt.Println("Eclipso is ready and serving zones from S3")
			return nil
		}
...
func TestE2E_TCPQuery(t *testing.T) {
	...
	r, _, err := c.Exchange(&m, eclipsoTCP)
	require.NoError(t, err)
	assert.Equal(t, dns.RcodeSuccess, r.Rcode)
	require.NotEmpty(t, r.Answer)
...
func TestE2E_DoTQuery(t *testing.T) {
	...
	r, _, err := c.Exchange(&m, eclipsoDoT)
	require.NoError(t, err)
	assert.Equal(t, dns.RcodeSuccess, r.Rcode)
	require.NotEmpty(t, r.Answer)
